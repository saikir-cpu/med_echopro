...
it.only('doctor should prescibe medication and other actionsto the outpatient neuro', () => {
  ...
  cy.get('.mt-4.flex > :nth-child(2)').click({ force: true });
  // --- Select From Date: 19 ---
...
cy.contains('label', 'Search Name (min 3 characters)')
  ...
  .type('P006910')
  cy.contains('h1', 'P006910')
  .closest('a') // Travel up to the parent anchor tag
  .click({ force: true })
// 1. Find the row containing the date/time
// 1. Find the stable text in the row (Date)
// ...existing code...

// Click the "+" button in Clinical Overview section
...
cy.get('input[placeholder="Quantity"]')
  ...
  cy.get('.MuiDialogActions-root > .font-rubik').click({ force: true });*/
